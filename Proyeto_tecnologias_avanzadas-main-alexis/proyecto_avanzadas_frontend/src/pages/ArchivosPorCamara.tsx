...
const ArchivosPorCamara = () => {
  ...
  useEffect(() => {
    if (!token || !idCamara) {
      setError("No autenticado o cámara no especificada");
      setLoading(false);
      return;
    }
    Promise.all([
      axios.get<FotoMeta[]>(`/api/imagenes/camara/${idCamara}`, { headers: { Authorization: `Bearer ${token}` } }),
      axios.get<VideoMeta[]>(`/api/videos/camara/${idCamara}`, { headers: { Authorization: `Bearer ${token}` } })
    ])
  ...
  return (
    <div className="flex gap-8">
      <div className="flex-1">
        <h2 className="text-2xl font-bold mb-4 text-blue-300">Fotos de la cámara</h2>
      ...
      <div className="flex-1">
        <h2 className="text-2xl font-bold mb-4 text-blue-300">Videos de la cámara</h2>
        <table className="min-w-full bg-[#23283a] rounded-lg overflow-hidden shadow border border-[#23283a]">
          <thead className="bg-[#23283a] text-blue-200">
            <tr>
              <th className="px-4 py-2">Vista</th>
